...
class MultioutputDiffGP(GP):
    """
    Multioutput Gaussian process model

    :param X_list: input observations in a list for each output
    :param Y_list: output observations in a list for each output
    :param kernel_list: GPy kernels in a list for each output
    :param likelihood_list: GPy likelihoods in a list for each likelihoods
    :param inference_method: The :class:`~GPy.inference.latent_function_inference.LatentFunctionInference` inference method to use for this GP
    :param kernel_cross_covariances: cross covariances between different outputs
    :rtype: model object
    ...
    """
    def __init__(self, X_list, Y_list, kernel_list, likelihood_list, name='multioutputdiffgp', kernel_cross_covariances={}, inference_method=None):
        #Input and Output
        X,Y,self.output_index = util.multioutput.build_XY(X_list,Y_list)
        Ny = len(Y_list)
        
        assert isinstance(kernel_list, list)
        kernel = kern.MultioutputDiffKern(kernels=kernel_list, cross_covariances=kernel_cross_covariances)
        ...
        super(MultioutputDiffGP, self).__init__(X,Y,kernel,likelihood, Y_metadata={'output_index':self.output_index}, inference_method = inference_method)# expectation_propagation.MultioutputEP()) # expectation_propagation.EP())                             
    ...
    def predict_noiseless(self,  Xnew, full_cov=False, Y_metadata=None, kern=None):
        ...
        return super(MultioutputDiffGP, self).predict_noiseless(Xnew, full_cov, Y_metadata, kern)
    ...
    def predict(self, Xnew, full_cov=False, Y_metadata=None, kern=None, likelihood=None, include_likelihood=True):
        if isinstance(Xnew, list):
            ...
            if Y_metadata is None:
                Y_metadata={'output_index': ind}
        return super(MultioutputDiffGP, self).predict(Xnew, full_cov, Y_metadata, kern, likelihood, include_likelihood)
    ...
    def predict_quantiles(self, X, quantiles=(2.5, 97.5), Y_metadata=None, kern=None, likelihood=None):
        if isinstance(X, list):
            ...
            if Y_metadata is None:
                Y_metadata={'output_index': ind}
        return super(MultioutputDiffGP, self).predict_quantiles(X, quantiles, Y_metadata, kern, likelihood)
